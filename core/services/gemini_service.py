...
class GeminiService:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        try:
            ...
            # Use gemini-2.5-flash (fast, free tier available)
...
"""
    ...
    @gemini_rate_limiter
    def explain_move_selection(
        self, 
        move: Dict[str, Any], 
        context: Dict[str, Any]
    ) -> str:
        """
        Generate natural language explanation for move selection.
        
        Args:
            move: Move metadata (move_label, difficulty, energy_level, etc.)
            context: Choreography context (parameters, previous moves, etc.)
            
        Returns:
            2-3 sentence explanation
        """
        move_label = move.get('move_label', 'Unknown move')
        difficulty = move.get('difficulty', 'unknown')
        energy_level = move.get('energy_level', 'unknown')
        
        # Get choreography parameters from context
        params = context.get('parameters', {})
        if isinstance(params, ChoreographyParameters):
            params = params.to_dict()
        
        requested_difficulty = params.get('difficulty', 'beginner')
        requested_style = params.get('style', 'romantic')
        
        prompt = f"""
You are a dance instructor explaining choreography choices to students.

Move: {move_label}
Move Difficulty: {difficulty}
Move Energy: {energy_level}

Student requested: {requested_difficulty} difficulty, {requested_style} style

Explain in 2-3 sentences why this move was selected. Focus on:
- How it matches their requested difficulty and style
- What makes it appropriate for their level
- How it contributes to the overall choreography flow

Be encouraging and educational. No markdown formatting.
"""
        ...
        try:
            ...
            logger.debug(f"Generated explanation for {move_label}")
            return explanation
            
        except Exception as e:
            logger.warning(f"Failed to generate explanation: {e}")
            # Fallback to template explanation
            return self._fallback_explanation(move, params)
    ...
    def _fallback_explanation(
        ...
        Args:
            move: Move metadata
            params: Choreography parameters
            
        Returns:
            Template explanation
        """
        move_label = move.get('move_label', 'this move')
        difficulty = params.get('difficulty', 'beginner')
        style = params.get('style', 'romantic')
        
        return (
            f"This {move_label} was selected because it matches your {difficulty} "
            f"level and {style} style preference. It flows naturally with the "
            f"music and provides good practice for fundamental bachata techniques."
        )
